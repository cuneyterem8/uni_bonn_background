...
def Ransac(edge_points):
	...
	while current_iter < max_iters and current_iter < N:
		...
		# initial_samples = ...
		initial_samples = np.zeros((2,s))
		randomly_drawn_indices = np.random.choice(edge_points.shape[1], s)
		for i in range(0,s):
			initial_samples[:,i] = edge_points[:,randomly_drawn_indices[i]]
		...
		# distance_to_center = ...
		#distance_to_center = 
		distance_to_center = np.linalg.norm(edge_points - center[np.newaxis].T, axis=0)
		
		# TODO: compare with model radius to determine if points are inliers or not (within margin)
		# inliers = ...
		max_dist = radius + distance_threshold
		min_dist = radius - distance_threshold
		#[ inlier_indices ] = np.where(distance_to_center <= max_dist)
		[ inlier_indices ] = np.where(np.logical_and(distance_to_center>=min_dist, distance_to_center<=max_dist)) 
		inliers = edge_points[:,inlier_indices]
		...
		eps = 1 - (num_inliers / edge_points.shape[1])

		# TODO: compute N
		# N = ...
		N = np.log(1-p) / np.log(1 - np.power((1-eps),s))
	...
	# new_edgepoints = ...
	max_dist = radius_best + distance_threshold
	distance_to_center = np.linalg.norm(edge_points - center_best[np.newaxis].T, axis=0)

	[ inlier_indices ] = np.where(distance_to_center <= max_dist)

	new_edgepoints = np.delete(edge_points, inlier_indices, 1)
