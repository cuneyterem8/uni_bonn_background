...
"""
@authors: Cüneyt Erem, Sheikh Mastura Farzana, Md Emtazul Haque
"""
...
def rayTracing(rayOrigin, rayDir):
    ...
    shifted_ray = M + N*eps
    for i, obj in enumerate(scene):
        hitShadowDistance = intersection(shifted_ray, DirtoLight, obj)
        if hitShadowDistance < hit:
            lightFactor = 0

    ###########################################################                
    # Computing the color, the default is ambient.
    color_ray = -5*color*obj.get('ambient', ambient)
    ...
    if(lightFactor == 1):
        color_ray = color*ambient*obj.get('ambient', ambient)
    
    obj_diffuse_c = obj.get('diffuse_c', diffuse_c)
    obj_specular_c = obj.get('specular_c', specular_c)
    obj_specular_k = obj.get('specular_k', specular_k)

    color_ray += color*diffuse_c*obj_diffuse_c*np.dot(DirtoLight, N)
    color_ray += specular_c*obj_specular_c*(np.dot(N, normalizer(DirtoCamera + DirtoLight))**(obj_specular_k))
...
for i, x in enumerate(np.linspace(Sc[0], Sc[2], width)):
    for j, y in enumerate(np.linspace(Sc[1], Sc[3], height)):
        ...
        while hit < hit_max:
            ...
            shifted_ray = M + N*eps
            reflected_ray = 2*np.dot(rayDir, N)*N

            rayOrigin = shifted_ray
            rayDir = rayDir - reflected_ray
