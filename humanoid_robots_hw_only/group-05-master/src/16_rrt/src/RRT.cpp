...
void RRT::addNearestNeighbor(AbstractNode* const currentNode, std::vector<AbstractNode*>& neighbors,
	...
	AbstractNode* const nearestNeighbor = getClosestNodeInList(randomNode, neighbors);
	if(nearestNeighbor != NULL) {
		nearestNeighbor->setPredecessor(currentNode);
		list.push_back(nearestNeighbor);
	}
...
std::deque<AbstractNode *> RRT::constructPath(AbstractNode * const connectionNode, AbstractNode * const startNode, AbstractNode * const goalNode) const {
	...
	for(AbstractNode* currentNode = connectionNode; currentNode != NULL; currentNode = currentNode->getPredecessor()) {
		path.push_front(currentNode);
	}

	for(AbstractNode* currentNode = connectionNode->getConnection(); currentNode != NULL; currentNode = currentNode->getPredecessor()) {
		path.push_back(currentNode);
	}

	if(startNode != path[0]) {
		std::deque<AbstractNode*> reversePath;
		for(size_t i = 0; i < path.size(); i++) {
			reversePath.push_front(path[i]);
		}
		return reversePath;
	}
	return path;
...
std::deque<AbstractNode *> RRT::planPath(AbstractNode * const startNode, AbstractNode * const goalNode, const size_t& maxIterations) {
	...
	std::vector<AbstractNode*> *currentList = &startList;
	std::vector<AbstractNode*> *otherList = &goalList;
	AbstractNode* currentGoal = goalNode;
	AbstractNode* otherGoal = startNode;

	for(size_t k = 0; k < maxIterations; k++) {
		AbstractNode* const randomNode = getRandomNode(*currentList, currentGoal);
		if(extendClosestNode(randomNode, *currentList, *otherList) != TRAPPED) {
			AbstractNode* const newNode = *currentList->rbegin();
			if(extendClosestNode(newNode, *otherList, *currentList) == REACHED) {
				return constructPath(connectionNode, startNode, goalNode);
			}
		}

		std::swap(currentList, otherList);
		std::swap(currentGoal, otherGoal);
	}
