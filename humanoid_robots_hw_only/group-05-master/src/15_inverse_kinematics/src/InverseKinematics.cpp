...
EndeffectorPose InverseKinematics_2Links::forwardKinematic(const JointAngles& q) const {
	...
	temp << h,
	        0,
	        1;
	temp = rotation(q(0))*translation(a0, 0)*rotation(q(1))*translation(a1, 0)* temp;
	e << temp(0),
		temp(1);
	return e;
}
...
Jacobian InverseKinematics_2Links::jacobian(const JointAngles& q) const {
	...
	/* Use the class member variables a0, a1, and h for the lengths given on the exercise sheet. */
	result << (h + a1)*(-sin(q(0))*cos(q(1)) - cos(q(0))*sin(q(1))) - a0 * sin(q(0)), (h + a1)*(-cos(q(0))*sin(q(1)) - sin(q(0))*cos(q(1))),
	(h + a1)*(cos(q(0))*cos(q(1)) - sin(q(0))*sin(q(1))) + a0 * cos(q(0)), (h + a1)*(-sin(q(0))*sin(q(1)) + cos(q(0))*cos(q(1)));
