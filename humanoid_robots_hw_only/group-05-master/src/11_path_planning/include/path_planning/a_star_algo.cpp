...
bool fileIO = true;
//FileIO *Open_list_class::fileIO = NULL;
//FileIO *Closed_list_class::fileIO = NULL;
...
//neighbors
std::vector<Node*> PathPlan::get_neighbor(const Node* current_node, const Map& map) {
...
std::deque<Node*> PathPlan::plan_path(Node* startNode, Node* goalNode, const Map& map) {
    ...
    while (!open_list_plan.is_empty()) {
        ...
        std::vector<Node*> neighbours = get_neighbor(current_node, map);
