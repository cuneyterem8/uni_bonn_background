...
std::vector<AbstractNode *> GridPathPlanning::getNeighborNodes(const GridNode * const currentNode, const GridMap& map) {
	std::vector<AbstractNode *> result;
            ...
            if(!map.isOccupied(currentNode->x-1,currentNode->y-1)){
                result.push_back( GridNode::get(currentNode->x-1,currentNode->y-1)); //1st neighbour
            }}
            if (currentNode->x>0){
            if(!map.isOccupied(currentNode->x-1,currentNode->y)){
                result.push_back( GridNode::get(currentNode->x-1,currentNode->y)); //2nd neighbour
            }}
            if(currentNode->x>0 && currentNode->y < map.width-1){
            if(!map.isOccupied(currentNode->x-1,currentNode->y+1)){
                result.push_back( GridNode::get(currentNode->x-1,currentNode->y+1)); //3rd neighbour
            }}
            if(currentNode->y > 0){
            if(!map.isOccupied(currentNode->x,currentNode->y-1)){
                result.push_back( GridNode::get(currentNode->x,currentNode->y-1)); //4th neighbour
            }}
            if(currentNode->y < map.width-1){
            if(!map.isOccupied(currentNode->x,currentNode->y+1)){
                result.push_back( GridNode::get(currentNode->x,currentNode->y+1)); //5th neighbour
            }}
            if(currentNode->x < map.height-1 && currentNode->y > 0){
            if(!map.isOccupied(currentNode->x+1,currentNode->y-1)){
                result.push_back( GridNode::get(currentNode->x+1,currentNode->y-1)); //6th neighbour
            }}
            if(currentNode->x < map.height-1){
            if(!map.isOccupied(currentNode->x+1,currentNode->y)){
                result.push_back( GridNode::get(currentNode->x+1,currentNode->y)); //7th neighbour
            }}
            if(currentNode->x < map.height-1 && currentNode->y< map.width-1){
            if(!map.isOccupied(currentNode->x+1,currentNode->y+1)){
...
void PathPlanning::expandNode(const AbstractNode * const currentNode, const AbstractNode * const goalNode,
        ...
        std::vector<AbstractNode *> neighbours = getNeighborNodes(currentNode);
...
std::deque<const AbstractNode*> PathPlanning::followPath(const AbstractNode * const node) {
	std::deque<const AbstractNode *> path;
	 const AbstractNode * temp = nullptr;
	 temp = node;
